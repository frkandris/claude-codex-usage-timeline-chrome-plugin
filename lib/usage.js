...
export function migrateCodexWindows(history) {
  ...
  const migrated = history.map((sample) => {
    ...
    if (!Number.isFinite(session?.used) || Number.isFinite(sample.codex.weekly?.used)) return sample;
    if (!Number.isFinite(session.resetsAt) || session.resetsAt - sample.timestamp <= CODEX_SESSION_WINDOW_MAX_MS) return sample;
    changed = true;
